...
public class SwerveTelemetry {
    ...
    public void telemeterize(SwerveDrivetrain.SwerveDriveState state) {
        var pose = state.Pose;
        // Retained for pose in simulation
        fieldTypePub.set("Field2d");
        fieldPub.set(new double[]{pose.getX(), pose.getY(), pose.getRotation().getDegrees()});
        Logger.recordOutput("Drive/Pose", new double[]{pose.getX(), pose.getY(), pose.getRotation().getDegrees()});
        ...
        Logger.recordOutput("Drive/Odometry Frequency", 1.0 / state.OdometryPeriod);
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            Logger.recordOutput("Drive/Swerve/Mechanism/" + i, moduleMechanisms[i]);
            Logger.recordOutput("Drive/Swerve/Speed/" + i, state.ModuleStates[i].speedMetersPerSecond);
            Logger.recordOutput("Drive/Swerve/Angle/" + i, state.ModuleStates[i].angle);
        }
